...
-- Initialize a polar vector from length and angle
function vector.vec2_polar(length, angle)
	return {length = length, angle = angle}
end

-- From polar to velocities
function vector.polar_to_vel(vec, L)
	v = vec.length
	w = vec.angle
	return {vl = (v)+(-L/2 * w), vr = (v)+(L/2 * w)}
end
